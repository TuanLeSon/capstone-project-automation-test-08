...
test.describe('Login Validation', () => {
  ...
  test('Empty Email & Password → show required validation', async ({ page }) => {
    ...
    await expect(login.accountValidationMsg).toHaveText(WarningMessages.MANDATORY_MSG);
    await expect(login.passwordValidationMsg).toHaveText(WarningMessages.MANDATORY_MSG);
  });
  ...
  test('Empty Password only → show password required message', async ({ page }) => {
    ...
    await login.account.fill('test@gmail.com');
    await login.submit.click();
